...
bool Initializer::ComputeG_R_IFromImuData(Eigen::Matrix3d* G_R_I) {
    ...
    const Eigen::Vector3d mean_acc = sum_acc / (double)imu_buffer_.size(); // 平均IMU加速度

    Eigen::Vector3d sum_err2(0., 0., 0.);
    for (const auto imu_data : imu_buffer_) {
        sum_err2 += (imu_data->acc - mean_acc).cwiseAbs2(); // (测量值-均值)^2,噪声项符合标准正态分布，因此求和后抵消了
    }
    const Eigen::Vector3d std_acc = (sum_err2 / (double)imu_buffer_.size()).cwiseSqrt(); // 方差 = acc bias
    ...
    // z-axis.
    const Eigen::Vector3d& z_axis = mean_acc.normalized(); // 加速度归一化后指向重力方向
